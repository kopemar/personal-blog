...
if (isset($_SESSION["admin"])&&$_SESSION["admin"]) { ?>
    ...
    <form id="edit" method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>?p=edit" novalidate>
        ...
        <?php } ?>
        <label> Nadpis:
            <input type="text" class="wide" name="edit_article_title" id="edit_article_title" placeholder="Nadpis"
                   value="<?php // if the user is editing, show him the contents of the article
                   if (isset($_POST["edit_article_title"])) {
                       echo htmlspecialchars($_POST["edit_article_title"]);
        ...
        <p>Autor: <?php //name of author cannot be changed in UI - who added it is now held in db
            if (isset($article)) {
                echo $article["author"];
            } else if (isset($_SESSION["user"])) echo $_SESSION["user"] ?></p>
        <p>Rubriky:
            <input type="hidden" name="category[]" value="">
            <?php foreach ($DB->getAllCategories() as $category) { ?>
                <label><?php echo htmlspecialchars($category["category"], ENT_QUOTES); ?> <input type="checkbox"
                                                                                                 name="category[]"
                                                                                                 value="<?php echo htmlspecialchars($category["category"], ENT_QUOTES) ?>"
        ...
        <label>Text<br>
            <textarea id="edit_article_text" placeholder="Text článku" name="edit_article_text"
                      required><?php if (isset($_POST["edit_article_text"])) {
                    echo htmlspecialchars($_POST["edit_article_text"]);
                } else if (isset($article)) {
                    echo htmlspecialchars($article["contents"]);
                } ?></textarea>
        </label>
        <br>
        <input type="submit" name="submit_article" value="Submit">
    </form>
    <?php if (isset($_POST["submit_article"])) {
        if (!titleValid($_POST["edit_article_title"]) && strlen($_POST["edit_article_title"]) == 0) { ?>
        ...
        <?php } else if (!textValid($_POST["edit_article_text"]) && strlen($_POST["edit_article_text"]) > 8000) {
            ?>
            <div class="error_message">Článek je moc dlouhý.</div>
        <?php } else if (isset($_POST["categories"])&&!checkSubmittedArticleCategories($_POST["categories"])) { ?>
            <div class="error_message">Některá z kategorií má moc dlouhý název. </div>
            <?php
        }
